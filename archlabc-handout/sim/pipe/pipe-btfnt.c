...
int gen_f_predPC()
{
    return (((if_id_next->icode) == (I_CALL)) ? (if_id_next->valc) : ((
      ...
      (((if_id_next->icode) == (I_JMP)) & ((if_id_next->valc) >= 
          (if_id_next->valp))) ? (if_id_next->valp) : (if_id_next->valp));
}
...
int gen_aluA()
{
    return (((id_ex_curr->icode) == (I_RRMOVL) || (id_ex_curr->icode) == 
        ...
        (I_MRMOVL) || (id_ex_curr->icode) == (I_JMP)) ? (id_ex_curr->valc)
       : ((id_ex_curr->icode) == (I_CALL) || (id_ex_curr->icode) == 
        (I_PUSHL)) ? -4 : ((id_ex_curr->icode) == (I_RET) || 
        (id_ex_curr->icode) == (I_POPL)) ? 4 : 0);
}
