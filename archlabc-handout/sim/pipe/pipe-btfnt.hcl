#林涛  1600012773
#要将valC和valP都传回predPC阶段，可以将valC, valP分别存在M_valE和M_valA中，
#通过旁路传到predPC阶段
#要修改f_predPC, f_pc和转移预测错误的异常判断条件
#f_predPC中需要的值有f_valC, f_valP
#f_pc中需要的有M_Cnd, M_valE, M_valA
#异常判断中需要的是：E_icode, e_Cnd, E_valC, E_valA
